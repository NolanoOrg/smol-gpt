...
struct ggml_tensor * ggml_t5_norm_impl(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        bool inplace) {
    bool is_node = false;
...
struct ggml_tensor * ggml_t5_norm(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    return ggml_t5_norm_impl(ctx, a, false);
}

struct ggml_tensor * ggml_t5_norm_inplace(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    return ggml_t5_norm_impl(ctx, a, true);
}
...
static void ggml_compute_forward_add_f32(
    ...
    if ((src0->ne[0] == 2 || src0->ne[1] == 2) && (src0->ne[0] == 1 || src0->ne[1] == 1) && src0->ne[2] == 12 && ith == 0 && ddddebug) {
        ddddebug = false;
        pretty_print_tensor((struct ggml_tensor *) src0);
        pretty_print_tensor((struct ggml_tensor *) src1);
        /*
            printf("Printing src0\n");
            char * src0_ptr = (char *) src0->data;
            for (int i = 0; i < src0->ne[3]; i++) {
                for (int j = 0; j < src0->ne[2]; j++) {
                    for (int k = 0; k < src0->ne[1]; k++) {
                        for (int l = 0; l < src0->ne[0]; l++) {
                            char * data_ptr = src0_ptr + (i * src0->nb[3]) + (j * src0->nb[2]) + (k * src0->nb[1]) + (l * src0->nb[0]);
                            printf("%.5f    ", *(float *) data_ptr);
                        }
                        printf("\n");
                    }
                    printf("\n");
            ...
            printf("Printing src1\n");
            char * src1_ptr = (char *) src1->data;
            for (int i = 0; i < src1->ne[3]; i++) {
                for (int j = 0; j < src1->ne[2]; j++) {
                    for (int k = 0; k < src1->ne[1]; k++) {
                        for (int l = 0; l < src1->ne[0]; l++) {
                            char * data_ptr = src1_ptr + (i * src1->nb[3]) + (j * src1->nb[2]) + (k * src1->nb[1]) + (l * src1->nb[0]);
                            printf("%.5f    ", *(float *) data_ptr);
                        }
                        printf("\n");
                    }
...
// ggml_compute_forward_t5_norm

static void ggml_compute_forward_t5_norm_f32(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        struct ggml_tensor * dst) {
    GGML_ASSERT(ggml_are_same_shape(src0, dst));
...
static void ggml_compute_forward_t5_norm(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * src0,
        struct ggml_tensor * dst) {
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_t5_norm_f32(params, src0, dst);
            } break;
        case GGML_TYPE_Q4_0:
        case GGML_TYPE_Q4_1:
        case GGML_TYPE_I8:
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_T5_NORM:
            {
                ggml_compute_forward_t5_norm(params, tensor->src0, tensor);
            } break;
