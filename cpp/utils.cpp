...
bool gpt_params_parse(int argc, char ** argv, gpt_params & params) {
    for (int i = 2; i < argc; i++) { // We increment by 2 because we skip the first two arguments - first being the model type
        ...
        if (arg == "-s" || arg == "--seed") {
            ...
            params.top_p = std::stof(argv[++i]) + 1e-5;
        } else if (arg == "--temp") {
            params.temp = std::stof(argv[++i]) + 1e-5;
        } else if (arg == "--repeat_last_n") {
            params.repeat_last_n = std::stoi(argv[++i]);
        } else if (arg == "--repeat_penalty") {
            params.repeat_penalty = std::stof(argv[++i]) + 1e-5;
...
std::vector<float> whitespace_tokenize_float(const std::string & prompt) {
    // The prompt is a sequence of numbers, separated by spaces.
